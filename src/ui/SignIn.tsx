...
export function SignIn({ onDone }: { onDone: (client: Client) => void }) {
  const [step, setStep] = useState<Step>({ name: "server" });
  const [serverUrl, setServerUrl] = useState("http://127.0.0.1:8484");
    ...
    <div className="signin-screen">
      ...
      {step.name === "server" && (
        <form onSubmit={connect} style={{ display: "contents" }}>
          <div className="signin-tagline">Connect to your BlitterServer</div>
          <input
