...
- **React 19 + TypeScript + Vite** in the webview (matches the musex UI being ported and the
  ...
  only the contract).
- The Rust host stays THIN: window/plugin/sidecar lifecycle only. App logic lives in React; music
  logic lives in BlitterServer.
...
- `src-tauri/` — Rust host: plugins (store, http, opener) and capabilities only.
